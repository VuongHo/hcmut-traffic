...
class SegmentController extends AbstractController
{
    ...
    public function post($request)
    {
        switch (count($request->url_elements)) {
        	case 1:
        		//http://localhost/api-framework/index.php/segmentdemo/speed		radius_x=11.5&radius_y=10.5&radius=7.5
        		if (trim($request->url_elements[0]) == "segment") {
	        		...
	        		$segment = $this->find_all_segment($street_id, $arr_segment_id);
        	...
        	case 2:
        		...
        		if (trim($request->url_elements[1]) == "speed"){
	        		...
	        		$arr_segment_id = join(',', $segment_id);
	        		$segment = $this->find_speed_segment($street_id, $arr_segment_id);
					
					return $segment;
        		}
        		elseif (trim($request->url_elements[1]) == "street"){
        			// get all parameters values from request
		        	$street_id = $request->parameters['street_id'];		// Street id	
					
	        		$segment = $this->find_segment_by_street_id($street_id);
